...
public class RT {
	...
	public static class InvokeStaticCallSite extends MutableCallSite {
		...
		public InvokeStaticCallSite(Lookup lookup, String name,
				MethodType type, Class<?> ownerType) {
			super(type);
			this.lookup = lookup;
			this.name = name;
	...
	public static CallSite invokestatic(Lookup lookup, String name,
			MethodType type, Class<?> ownerType) {
		InvokeStaticCallSite callsite = new InvokeStaticCallSite(lookup, name,
				type, ownerType);
		callsite.setTarget(callsite.staticSwitch());
		return callsite;
	}

	public static Object staticFallback(InvokeStaticCallSite callSite,
	...
	public static class InvokeVirtualCallSite extends MutableCallSite {
		...
		public InvokeVirtualCallSite(Lookup lookup, String name, MethodType type) {
			...
			MethodHandle checkClass = MethodHandles.dropArguments(CHECK_CLASS,
					2, type.dropParameterTypes(0, 1).parameterList());
			checkClass = checkClass.asType(type().changeReturnType(
					boolean.class).insertParameterTypes(0, Class.class));
			this.checkClass = checkClass;

			MethodHandle switchFallback = MethodHandles.insertArguments(
					VIRTUAL_FALLBACK, 0, this, true);
			switchFallback = switchFallback.asCollector(Object[].class,
					type.parameterCount());
			switchFallback = switchFallback.asType(type);
			this.switchFallback = switchFallback;

			MethodHandle dispatchFallback = MethodHandles.insertArguments(
					VIRTUAL_FALLBACK, 0, this, false);
			dispatchFallback = dispatchFallback.asCollector(Object[].class,
					type.parameterCount());
	...
	public static CallSite invokevirtual(Lookup lookup, String name,
			MethodType type) {
		InvokeVirtualCallSite callSite = new InvokeVirtualCallSite(lookup,
				name, type);
	...
	public static Object virtualFallback(InvokeVirtualCallSite callSite,
			boolean reset, Object[] args) throws Throwable {
		Class<?> receiverClass = args[0].getClass();
		MethodHandle virtualSwitch = callSite.virtualSwitch(receiverClass);

		MethodHandle fallback = (reset) ? callSite.dispatchFallback : callSite
				.getTarget();

		MethodHandle guard = MethodHandles.guardWithTest(
				callSite.checkClass.bindTo(receiverClass), virtualSwitch,
				fallback);
		...
		try {
			STATIC_FALLBACK = lookup.findStatic(RT.class, "staticFallback",
					MethodType.methodType(Object.class,
							InvokeStaticCallSite.class, Object[].class));
			VIRTUAL_FALLBACK = lookup.findStatic(RT.class, "virtualFallback",
					MethodType.methodType(Object.class,
							InvokeVirtualCallSite.class, boolean.class,
							Object[].class));
			CHECK_CLASS = lookup.findStatic(RT.class, "checkClass", MethodType
