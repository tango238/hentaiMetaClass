...
public class MetaClass {
	...
	private static final ClassValue<MetaClass> metaClassValue = new ClassValue<MetaClass>() {
		@Override
		protected MetaClass computeValue(Class<?> type) {
			Class<?> superclass = type.getSuperclass();
			MetaClass parentMetaClass = (superclass == null) ? null
					: getMetaClass(superclass);
	...
	private final LinkedList<WeakReference<MetaClass>> subMetaClasses = new LinkedList<WeakReference<MetaClass>>();
	private final HashMap<Selector, MethodHandle> vtable = new HashMap<Selector, MethodHandle>();
	...
	public void redirect(String name, MethodType type, MethodHandle target) {
		synchronized (MUTATION_LOCK) {
			...
			SwitchPoint.invalidateAll(switchPoints
					.toArray(new SwitchPoint[switchPoints.size()]));

			vtable.put(new Selector(name, type), target);
		}
	}

	private static void mutateSwitchPoints(MetaClass metaClass,
			ArrayList<SwitchPoint> switchPoints) {
		assert Thread.holdsLock(MUTATION_LOCK);

		switchPoints.add(metaClass.switchPoint);
		metaClass.switchPoint = new SwitchPoint(); // new switch point

		for (Iterator<WeakReference<MetaClass>> it = metaClass.subMetaClasses
				.iterator(); it.hasNext();) {
